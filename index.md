---
layout: home
title: About
---

I am an assistant professor of International Relations and Korean Studies at [Leiden University](https://www.universiteitleiden.nl/) and an affiliated researcher with the [Leiden University Centre for Digital Humanities (LUCDH)](https://www.universiteitleiden.nl/en/humanities/centre-for-digital-humanities). I earned a Ph.D. in Political Science from the University of Toronto, an M.A. in Global Affairs and Policy from Yonsei University, and a B.A. in Political Science from Harding University.

I study how political institutions shape and are shaped by national identity, migration, and political behavior. My work sits at the intersection of comparative politics and digital humanities, drawing on surveys, experiments, and computational text analysis. My regional focus is East Asia, especially the Korean Peninsula, although my comparative and applied work extends to other jurisdictions.

Increasingly, my research relies on building research corpora from archival and other hard-to-use sources, combining OCR, vision-language models, and large language models to work with bodies of text that were previously difficult to analyze. I also conduct applied policy research on innovation policy, firm growth, and labour markets.

My teaching covers many of the same methods. At Leiden I teach computational text analysis and digital humanities, as well as courses in research design and international relations. I supervise BA and MA theses across four programs in International Studies, Korean Studies, Asian Studies, and International Relations. I also supervise PhD research and accept new candidates on a rolling basis.

I also develop and maintain open educational resources for research methods and computational text analysis, including curated Korean-studies corpora and open-source tools for experimental design and pre-submission manuscript review. Alongside these, I write and maintain [*Pixels and Patterns*](https://www.pixelsandpatterns.org/), a Substack on how to integrate and use AI responsibly in research and other computational work.

Beyond Leiden, I serve as Director of Research at [SinoNK.com](https://sinonk.com), a Senior Fellow at the [European Centre for North Korean Studies](https://ecnk.univie.ac.at/) at the University of Vienna, and a Non-Resident Fellow in the [Innovation Policy Lab](https://munkschool.utoronto.ca/ipl/) at the University of Toronto. These affiliations reflect my commitment to publicly engaged scholarship and policy-relevant research.

For more, see [Projects](/projects/) for an overview of active and recently completed projects, [Research](/research/) for an overview of published work and working papers, and [Teaching](/teaching/) for links to my teaching resources.
